...
- Includes GRUB EL2 menu entry
- Includes \`slbounceaa64.efi\`, \`qebspilaa64.efi\`, \`tcblaunch.exe\`, and DSP firmware blobs in ESP \`/firmware/qcom/sc8280xp/HUAWEI/gaokun3/\`
...
${EL2_LOGIN_BLOCK}${reassemble_block}
EOF
}
...
TAG_NAME="archlinuxarm-${KREL}$(if [[ "$BUILD_EL2" == "true" ]]; then printf -- '-el2'; fi)-$(date -u +%Y%m%d%H%M%S)"
